    ...
    def run(self, pages: list[dict]) -> SkillResult:
        ...
        for page in pages:
            ...
            for og_key, label in [("og:title", og_title), ("og:description", og_desc),
                ...
                if not label:
                    # og:title and og:url directly affect social card rendering — warning.
                    # og:description and og:image are strongly recommended but non-breaking — info.
                    sev = "warning" if og_key in ("og:title", "og:url") else "info"
                    findings.append(Finding(
