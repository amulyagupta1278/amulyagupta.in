...
def load_email_log() -> list:
    ...
    if not isinstance(data, list):
        log.error("emails.json is corrupted (not a list) — resetting email log")
        return []
    return data
...
def build_cwv_summary(findings: list, skill_metadata: dict = None) -> dict:
    """Build CWV summary from skill metadata (primary) or finding categories (fallback)."""
    cwv: dict = {"lcp": [], "cls": [], "fid": [], "inp": [], "ttfb": [], "records": []}

    # Primary source: structured records from Skill 5 / Skill 15 metadata
    records = (skill_metadata or {}).get("cwv_records", [])
    for rec in records:
        for metric, key in [("lcp", "lcp_ms"), ("cls", "cls"),
                            ("fid", "fid_ms"), ("inp", "inp_ms"), ("ttfb", "ttfb_ms")]:
            val = rec.get(key)
            if val is not None:
                try:
...
def build_dashboard_snapshot(run: dict, findings: list, scores: list, issues: dict,
    ...
    # CWV summary — use skill metadata when available (Skill 5/15), else empty
    cwv_summary = build_cwv_summary(findings, skill_metadata=skill_metadata)
