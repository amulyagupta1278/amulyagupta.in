...
def build_cwv_summary(findings: list, metadata: dict = None) -> dict:
    """Extract Core Web Vitals from skill metadata (cwv_records from Skills 5/15)."""
    cwv = {"lcp": [], "cls": [], "fid": [], "inp": [], "ttfb": [], "records": []}

    # CWV numbers live in the skill SkillResult.metadata, not in individual findings.
    # Skill 05 / Skill 15 store them under the "cwv_records" key.
    for rec in (metadata or {}).get("cwv_records", []):
        for metric, key in [
...
def build_dashboard_snapshot(run: dict, findings: list, scores: list, issues: dict, metadata: dict = None) -> dict:
    now = datetime.utcnow().isoformat()
    ...
    cwv_summary = build_cwv_summary(findings, metadata)
